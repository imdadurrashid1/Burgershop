...
const transporter = nodemailer.createTransport({
    service: 'gmail',
    auth: {
        user: process.env.EMAIL_USER || 'imdadurburger@gmail.com', // Replace with your email address
...
const sendVerificationEmail = async (email, otp, name) => {
    ...
    try {
        const mailOptions = {
            from: `Imdadur Burger Shop <${process.env.EMAIL_USER || 'imdadurburger@gmail.com'}>`,
            to: email,
            subject: 'Your Verification Code for Imdadur Burger Shop',
            html: `
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 5px;">
                    <div style="text-align: center; margin-bottom: 20px;">
                        <h1 style="color: #f44336;">Imdadur Burger Shop</h1>
                    </div>
                    ...
                    <div style="text-align: center; font-size: 12px; color: #757575; margin-top: 30px;">
                        <p>&copy; ${new Date().getFullYear()} Imdadur Burger Shop. All rights reserved.</p>
                    </div>
                </div>
            `
        };
